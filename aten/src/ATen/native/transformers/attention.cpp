...
_scaled_dot_product_flash_attention_backward_cpu(
    ...
    const Tensor& philox_offset,
    c10::optional<double> scale) {
  auto grad_results = _scaled_dot_product_flash_attention_mask_backward_cpu(
  ...
  return std::make_tuple(std::get<0>(grad_results), std::get<1>(grad_results), std::get<2>(grad_results));
}
