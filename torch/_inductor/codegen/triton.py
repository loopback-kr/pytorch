...
class TritonKernel(Kernel):
    ...
    def codegen_kernel_benchmark(self):
        ...
        with result.indent():
            result.writeline(f"with {V.graph.deviceOpOverrides.DeviceGuard(index)}:")
            with result.indent():
                result.writeline(
                    V.graph.deviceOpOverrides.set_device(index)
        ...
        with result.indent():
            result.writeline(f"with {V.graph.deviceOpOverrides.DeviceGuard(str(index))}:")
            with result.indent():
                result.writeline(
                    V.graph.deviceOpOverrides.set_device(index)
                )  # no-op to ensure context
...
class TritonScheduling(BaseScheduling):
    ...
    def codegen_sync(self):
        V.graph.wrapper_code.writeline(V.graph.deviceOpOverrides.synchronize())
